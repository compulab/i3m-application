...
static void sprintf_rtc_hour(struct gfx_information *info, char *output_str)
{
	if (calendar_is_date_valid(&computer_date_time))
		sprintf(output_str, "%02d", computer_date_time.hour);
	else
...
static void sprintf_rtc_min(struct gfx_information *info, char *output_str)
{
	if (calendar_is_date_valid(&computer_date_time))
...
static void sprintf_rtc_sec(struct gfx_information *info, char *output_str)
{
